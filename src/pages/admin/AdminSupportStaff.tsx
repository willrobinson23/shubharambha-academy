...
const AdminSupportStaff = () => {
    ...
    mutationFn: async () => {
      ...
      if (file) {
        try {
          ...
          const fileName = `staff_${Math.random()}.${fileExt}`;
      ...
      const staffData = { name, role, image_url: finalImageUrl };

      if (editId) {
        const { error } = await supabase.from("support_staff").update(staffData).eq("id", editId);
        if (error) throw error;
      } else {
        const { error } = await supabase.from("support_staff").insert([staffData]);
        if (error) throw error;
      }
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["admin-support-staff"] });
      toast({ title: editId ? "Staff member updated successfully!" : "Staff member added successfully!" });
      resetForm();
      setDialogOpen(false);
      setIsUploading(false);
    },
  ...
  const deleteMutation = useMutation({
    ...
    onError: (err: any) => {
      toast({ title: "Failed to remove staff member", description: err.message, variant: "destructive" });
    }
  });
  ...
  return (
    <AdminLayout>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-display font-bold">Manage Support Staff</h1>
          ...
          <DialogContent className="sm:max-w-lg">
            ...
            <form onSubmit={handleSubmit} className="space-y-6 pt-4">
              ...
              <Button type="submit" className="w-full" disabled={saveMutation.isPending || isUploading}>
                {saveMutation.isPending || isUploading ? "Saving..." : editId ? "Update Staff Member" : "Add to Support Staff"}
              </Button>
            </form>
          </DialogContent>
        </Dialog>
                    ...
                    <AlertDialogContent>
                      <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                          This will remove them from the support staff section forever.
                        </AlertDialogDescription>
                      </AlertDialogHeader>
                      <AlertDialogFooter>
