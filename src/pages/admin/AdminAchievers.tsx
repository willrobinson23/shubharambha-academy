...
const AdminAchievers = () => {
  ...
  const addAchieverMutation = useMutation({
    mutationFn: async (urlToSave: string) => {
      const { error } = await supabase.from("achievers").insert([
        { name, passed_class: passedClass, gpa, image_url: urlToSave }
      ]);
      if (error) throw error;
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["admin-achievers"] });
      toast({ title: "Top achiever added successfully!" });
      setName("");
      setPassedClass("");
      setGpa("");
      setImageUrl("");
    ...
    onError: (err) => {
      toast({ title: "Failed to add top achiever", description: err.message, variant: "destructive" });
      setIsUploading(false);
    }
  });
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    if (!name.trim() || !passedClass.trim() || !gpa.trim()) {
      toast({ title: "Name, Passed Class, and GPA are required.", variant: "destructive" });
      return;
    }
    ...
    addAchieverMutation.mutate(finalImageUrl);
  };

  return (
    <AdminLayout>
      <h1 className="text-2xl font-display font-bold mb-6">Manage Top Achievers</h1>

      <Card className="mb-8">
        <CardContent className="p-6">
          <h2 className="text-lg font-semibold mb-4 flex items-center gap-2">
            <Users className="h-5 w-5" /> Add New Achiever
          </h2>
          <form onSubmit={handleAdd} className="space-y-6">
            ...
            <Button type="submit" disabled={addAchieverMutation.isPending || isUploading}>
              {addAchieverMutation.isPending || isUploading ? "Adding..." : "Add to Achievers"}
            </Button>
          </form>
        </CardContent>
      </Card>
          ...
          {achievers.map((member) => (
            <Card key={member.id} className="overflow-hidden group relative flex flex-col">
              ...
              <div className="p-4 bg-background">
                ...
                <p className="text-muted-foreground text-sm font-medium">{member.passed_class} • GPA: {member.gpa}</p>
              </div>
            </Card>
          ))}
        </div>
